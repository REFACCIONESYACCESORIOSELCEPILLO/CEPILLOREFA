...
publicWidget.registry.sh_motorcycle_shop_search = publicWidget.Widget.extend({
  ...
  start: function () {
    ...
    self.$el.find(".id_sh_motorcycle_type_select > option").not(".blank").remove();
    rpc("/sh_motorcycle/get_type_list").then(function (data) {
      jQuery.each(data, function (key, value) {
        self.$el.find(".id_sh_motorcycle_type_select").append(
    ...
    this.save_bike_to_garage_btn_old_style = self.$el.find(
    ...
    rpc("/sh_motorcycle/is_bike_already_in_garage", {
      ...
      if (rec.is_bike_already_in_garage) {
        self.$el.find(".id_sh_motorcycle_save_bike_to_garage_btn").hide();
      } else {
        self.$el.find(".id_sh_motorcycle_save_bike_to_garage_btn").show();
      }
    });

    self.$el.find(".id_sh_motorcycle_select_saved_bike_div > a").remove();

    rpc("/sh_motorcycle/get_saved_bike").then(function (data) {
      jQuery.each(data, function (key, value) {
        self.$el.find(".id_sh_motorcycle_select_saved_bike_div").append(
  ...
  _onChangeTypeGetMake: function (ev) {
    var self = this;
    //ev.preventDefault();
    self.$el.find(".id_sh_motorcycle_make_select > option").not(".blank").remove();
    
    rpc("/sh_motorcycle/get_make_list", {
      type_id: self.$el.find(".id_sh_motorcycle_type_select > option:selected").val(),
    }).then(function (data) {
      jQuery.each(data, function (key, value) {
        self.$el.find(".id_sh_motorcycle_make_select").append(
          '<option value="' + value.id + '">' + value.name + "</option>"
        );
      });
  ...
  _onChangeMakeGetModel: function (ev) {
    var self = this;
    //ev.preventDefault();
    self.$el.find(".id_sh_motorcycle_model_select > option").not(".blank").remove();
    rpc("/sh_motorcycle/get_model_list", {
      type_id: self.$el.find(".id_sh_motorcycle_type_select > option:selected").val(),
      make_id: self.$el.find(".id_sh_motorcycle_make_select > option:selected").val(),
    }).then(function (data) {
      jQuery.each(data, function (key, value) {
        self.$el.find(".id_sh_motorcycle_model_select").append(
          '<option value="' + value.id + '">' + value.name + "</option>"
        );
      });
      //self.diable_select_options();
  ...
  _onChangeModelGetYear: function (ev) {
    ...
    self.$el.find(".id_sh_motorcycle_year_select > option").not(".blank").remove();
    rpc("/sh_motorcycle/get_year_list", {
      type_id: self.$el.find(".id_sh_motorcycle_type_select > option:selected").val(),
      make_id: self.$el.find(".id_sh_motorcycle_make_select > option:selected").val(),
      model_id: self.$el.find(".id_sh_motorcycle_model_select > option:selected").val(),
    }).then(function (data) {
      jQuery.each(data, function (kqy, value) {
        self.$el.find(".id_sh_motorcycle_year_select").append(
